...
export default function PromotionalBanner() {
  ...
  const handleFileChange = (e) => {
    ...
    const oversized = picked.filter((f) => f.size > 2 * 1024 * 1024)
    if (oversized.length) {
      alert(`Image size must be 2MB or less: ${oversized.map((f) => f.name).join(", ")}`)
      return
    }

    // Editing replaces the single existing banner; creating can add several at once.
    const files = editingBanner ? picked.slice(0, 1) : picked
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true)
      const selected = formData.files.length ? formData.files : (formData.file ? [formData.file] : [])

      const data = new FormData()
      // The bulk route reads "files"; the single route reads "file".
      selected.forEach((f) => data.append("files", f))
      if (selected[0]) data.append("file", selected[0])
      ...
      if (editingBanner) {
        ...
        res = await api.post("/food/hero-banners/home-promotion/multiple", data)
      }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                      ...
                      {formData.preview ? (
                        ...
                        <div className="text-center p-4">
                          ...
                          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">
                            {editingBanner ? "Upload 5:1 Image" : "Upload 5:1 Images"}
                          </p>
                        </div>
                      )}
                      <input
                    ...
                    <p className="text-[10px] text-slate-400">
                      ...
                      {!editingBanner && " Select several to add multiple banners — they rotate on the user home page."}
                    </p>

                    {!editingBanner && formData.files.length > 0 && (
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {formData.files.map((f, i) => (
                            <div key={`${f.name}-${i}`} className="relative w-14 h-14 rounded-lg overflow-hidden border border-slate-200 group/thumb">
