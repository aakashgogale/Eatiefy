...
export const createInboxNotifications = async ({ notifications = [] } = {}) => {
    ...
    const operations = rows.map((item) => {
        const payload = {
            ...
            source: item.source && ['ADMIN_BROADCAST', 'FSSAI_EXPIRY', 'ORDER_UPDATE', 'RESTAURANT_APPROVAL', 'DELIVERY_PARTNER_APPROVAL'].includes(item.source) ? item.source : 'ADMIN_BROADCAST',
